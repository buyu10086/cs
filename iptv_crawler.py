...
def filter_cctv_channels(iptv_content: str, collected_channels: Dict[str, List[str]]) -> None:
    """优化：放宽筛选规则，增加中文名称匹配，提升频道提取数量"""
    # 优化1：扩展正则，支持更多格式（无换行、带额外参数）
    ...
    for name, url in chinese_cctv_pattern.findall(iptv_content):
        name = name.strip()
        # 中文名称转换为统一格式（如 央视1套 → CCTV-1 综合）
        if re.search(r'1套|综合', name):
            name = "CCTV-1 综合频道"
        elif re.search(r'新闻', name):
            name = "CCTV-13 新闻频道"
        collected_channels.setdefault(name, []).append(url.strip())
...
def test_source_speed(source_url: str) -> float:
    ...
    try:
        ...
        # 优化1：用 GET 请求替代 HEAD 请求，兼容更多流媒体源
        response = requests.get(
            source_url,
            timeout=TIMEOUT,
            allow_redirects=True,
            ...
            stream=True  # 流式请求，不下载完整内容，仅获取响应头
        )
        response_time = (datetime.datetime.now() - start_time).total_seconds()
        
        # 优化2：放宽状态码要求，支持 200/206/301/302（重定向也是有效源）
        valid_status_codes = (200, 206, 301, 302, 403)  # 403 部分源权限足够播放
        return response_time if response.status_code in valid_status_codes else float('inf')
    except Exception:
        # 优化3：不直接返回极大值，给慢源一个机会（避免完全丢弃）
        return 10.0  # 替代 inf，标记为慢源但保留

def auto_select_fast_source(channel: Dict) -> Dict:
    """优化：即使无最快源，也保留频道（用第一个可用源），避免频道丢失"""
...
def generate_m3u8_playlist(valid_channels: List[Dict], filename: str = "iptv_playlist.m3u8") -> None:
    """生成 m3u8 文件，优化：保留所有有源的频道，不丢弃"""
    # 优化：筛选有「最快源」或「默认源」的频道
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write("# 生成说明：CCTV 频道列表（含测速优先源和默认源）\n")
        f.write("# 兼容播放器：VLC、PotPlayer、Kodi\n")
    ...
    print(f"   包含 {len(usable_channels)} 个 CCTV 频道（大幅提升频道数量）")

def main():
    """主流程：优化后提升频道提取和生成率"""
    print("=== 开始执行 CCTV 频道筛选（优化版，提升生成数量）===")
    collected_cctv = {}
